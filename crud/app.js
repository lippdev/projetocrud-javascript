...
newMemberAddBtn.addEventListener('click', ()=> {
    isEdit = false
    submitBtn.innerHTML = "Pronto"
    modalTitle.innerHTML = "Complete o Formulário"
    modalTitle.style.color = "#dae0e7"
    popupFooter.style.display = "block"
    imgInput.src = "./imagem/pic1.png"
...
function highlightIndexBtn(){
    ...
    entries.textContent = `Mostrando ${startIndex} de ${endIndex} de ${arrayLength} unidades`
...
function editInfo(id, pic, fname, lname, Age, City, Position, Salary, SDate, Email, Phone){
    ...
    // Procurar o index original do item para editá-lo
    const originalIndex = originalData.findIndex(item => item.id === id)

    // Atualizar a original data
    ...
    popupForm.classList.add('active')
    popupFooter.style.display = "block"
    modalTitle.innerHTML = "Atualizar Formulário"
    modalTitle.style.color = "#dae0e7"
...
function deleteInfo(index){
    if(confirm("Você tem certeza que quer deletar?")){
        ...
        // Atualizar getdata depois de deletar
...
filterData.addEventListener("input", ()=> {
    ...
    if(searchTerm !== ""){
        ...
        // Atualizar a current data com a filteredData
